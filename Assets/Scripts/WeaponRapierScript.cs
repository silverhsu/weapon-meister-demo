...
public class WeaponRapierScript : WeaponControlScript
{
    [HideInInspector]
    ...
    public override void Update()
    {
        ...
        if (isAtkOnCooldown)
        {
            ...
            this.transform.position = playerScript.neutralHandTrans.position + (playerScript.neutralHandTrans.up * 0.85f) + (playerScript.neutralHandTrans.right * 0.25f);
        }
    }
    ...
    public override void OnWeaponButtonReleased()
    {
        if (!isAtkOnCooldown)
        {
            ...
            Instantiate(attackPrefab, playerScript.transform.position + (new Vector3(playerScript.inputRightStick.x, playerScript.inputRightStick.y, 0f) * 1.5f), Util.UnitVectorToQuaternion(playerScript.inputRightStick));
